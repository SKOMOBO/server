...
var self = {
    ...
    /**
     * This  creates our tables and database if it doesn't already exist
     * 
     */
     setup_db(){
    ...
    // so that we only get x number of rows will have to calculate chunks
     ...
     get_box(id, resp, format){
        ...
        self.connection.query(query, (err, results , fields)=>{ 
            if(results != null){
                if(results.length !== 0){
                    if(format === 'json'){
        ...
        if(!has(values, null)){
            box_exists(values["Box_ID"], (exists)=>{
                if(exists){
                    ...
                    self.connection.query(`CREATE TABLE ${database_name} LIKE box_data`,  (err, results, fields)=>{
                        self.connection.query(`ALTER TABLE ${database_name} AUTO_INCREMENT = 1`, (err, results, fields)=>{
                            ...
                            let query = `Insert into ${database_name} ( ${cols} ) SELECT ${cols} from arduino where Box_ID = '${values["Box_ID"]}'`
                            ...
                            self.connection.query(query, (err, results , fields)=>{
                                ...
                                // insert new data
                                self.connection.query('INSERT INTO box_info set ?' , box_meta)
    ...
    get_connection(){
     ...
     get_info(id, cols, callback){
        ...
        if(id != null){
            ...
            cols = cols.join(', ')
            self.connection.query("select " + cols + " from box_info where id = '" + String(id) + "'", (err, results , fields)=>{
